...
const initialFormData: ProductFormData = {
  ...
  channelId: "",
  _styles: {
    displayName: "w-full px-3 py-2 bg-[hsl(var(--dark-lighter))] border border-gray-600/30 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))]",
    subscribers: "w-full px-3 py-2 bg-[hsl(var(--dark-lighter))] border border-gray-600/30 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))]"
  }
};
...
export default function ProductForm() {
  ...
  const fetchYoutubeChannelData = async (url: string) => {
    try {
      ...
      const channelIdentifier = extractChannelIdFromUrl(url);
      ...
      const channelExists = await checkIfChannelExists(url);
      if (channelExists) {
      ...
      // If the identifier starts with @, we need to get the channel ID first
      if (channelIdentifier.startsWith('@')) {
        const username = channelIdentifier.substring(1);
        console.log("Searching for username:", username);
        
        // Use channels.list with forUsername parameter
        const channelUrl = `https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&forUsername=${username}&key=${apiKey}`;
        ...
        if (channelData.items && channelData.items.length > 0) {
          ...
          console.log("No channel found with forUsername, trying search API");
          // If not found with forUsername, try search API as fallback
          const searchUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${username}&type=channel&key=${apiKey}`;
        ...
        const searchData = await searchResponse.json();
          ...
          if (searchData.items && searchData.items.length > 0) {
            ...
            // Fetch full channel data with the found channel ID
            const fullChannelUrl = `https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&id=${channelId}&key=${apiKey}`;
            const fullResponse = await fetch(fullChannelUrl);
            channelData = await fullResponse.json();
        ...
        // If we have a direct channel ID, fetch the data
        const channelUrl = `https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&id=${channelId}&key=${apiKey}`;
        console.log("Direct channel URL:", channelUrl);
        
        const response = await fetch(channelUrl);
        channelData = await response.json();
        console.log("Direct channel data:", channelData);
      }
      
      if (channelData.items && channelData.items.length > 0) {
        const channel = channelData.items[0];
        console.log("Processing channel data:", channel);
        ...
        // Set the basic channel data first
        setFormData(prev => ({
          ...prev,
          displayName: channel.snippet.title || "",
          subscribers: parseInt(channel.statistics.subscriberCount) || 0,
          channelId: channelId,
          channelLogo: channelLogo, // Set the original logo URL directly
          // Add custom styles for auto-filled fields
          _styles: {
            displayName: "w-full px-3 py-2 bg-[hsl(var(--dark-lighter))] border border-gray-600/30 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))]",
            subscribers: "w-full px-3 py-2 bg-[hsl(var(--dark-lighter))] border border-gray-600/30 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))]"
          }
        ...
        // Try to save the logo in the background, but don't wait for it
        if (channelLogo && user && channelId) {
          // Start the logo saving process but don't wait for it
          (async () => {
          try {
            // First download and store the logo
            ...
            // Then save the logo information to Firestore
              try {
            await saveChannelLogo(
              ...
              'YouTube'
                ...
                // Update the form with the stored logo URL only if both operations succeed
            setFormData(prev => ({
              ...prev,
                  channelLogo: storedLogoUrl
            }));
            
            console.log("Saved channel logo to storage, URL:", storedLogoUrl);
              } catch (firestoreError) {
                console.error("Error saving to Firestore:", firestoreError);
                // Keep using the original YouTube logo URL
              }
            } catch (storageError) {
              console.error("Error storing logo:", storageError);
              // Keep using the original YouTube logo URL
            }
          })();
        }
...
$${formData.expenses} — expense (month)`;
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-[hsl(var(--dark-card))] rounded-lg shadow-xl p-6 md:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          <select
            ...
            required
              className="w-full px-3 py-2 bg-[hsl(var(--dark-lighter))] border border-gray-600/30 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))]"
          >
            {platforms.map((platform) => (
                <option key={platform} value={platform} className="bg-[hsl(var(--dark-card))]">{platform}</option>
            ))}
          </select>
        </div>
        ...
        <div className="relative">
          <input
            ...
            required
              className={`w-full px-3 py-2 bg-[hsl(var(--dark-lighter))] border ${isChannelAlreadyUploaded ? 'border-red-500/50 bg-red-500/10' : 'border-gray-600/30'} rounded-md text-white focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))]`}
          />
          {isLoading && (
        ...
        {!isValidChannel && formData.platform === "YouTube" && (
          <div className="flex-1">
            <input
              ...
              placeholder="Price ($)"
              required
                className="w-full px-3 py-2 bg-[hsl(var(--dark-lighter))] border border-gray-600/30 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
          </div>
        )}
        ...
        <div className="flex justify-center pt-4">
          <button
            ...
            className={`px-6 py-2 ${
              ...
              } text-white font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))] focus:ring-offset-2 focus:ring-offset-[hsl(var(--dark-card))] disabled:opacity-50 transition-colors flex items-center justify-center min-w-[180px]`}
          >
