...
function MyChatsContent() {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-[#212121]">
      {/* Header Banner */}
      <div className="bg-[#212121] py-4 px-6 relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:right-0 after:h-[1px] after:bg-white/20">
        <div className="max-w-full w-full mx-auto">
          <div className="flex items-center justify-between">
            <Link 
              href="/"
              className="text-2xl font-bold w-full text-center hover:opacity-80 transition-colors duration-200 cursor-pointer"
            >
            ...
            <Link 
              href="/"
              className="px-4 py-2 bg-[#212121] text-gray-200 rounded-md hover:bg-gray-800 transition-colors duration-150 flex items-center shadow-sm font-medium"
            >
      ...
      {/* Chat Interface */}
      <div className="flex flex-1 overflow-hidden w-full mx-0 rounded-none shadow-lg bg-[#212121]">
        {/* Chat List on the left */}
        <div className="w-full md:w-1/3 lg:w-1/4 xl:w-1/5 h-full flex flex-col border-r border-gray-800">
          <div className="flex-grow overflow-y-auto bg-[#212121]">
          ...
          {selectedChatId && (
            <div className="p-4 bg-[#212121] border-t border-gray-800">
              <button 
                ...
                className="w-full bg-[#212121] text-gray-200 hover:bg-gray-800 font-medium py-2.5 px-4 rounded-md shadow-sm hover:shadow transition-all duration-150 flex items-center justify-center"
              >
        ...
        <div className="flex-1 h-full bg-[#212121]">
          {selectedChatId ? (
...
function EnhancedChatList({ onChatSelect, selectedChatId }: { onChatSelect: (chatId: string, productId: string) => void, selectedChatId: string | null }) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-12 bg-[#212121]">
  ...
  if (chats.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-12 px-4 bg-[#212121]">
        <div className="w-20 h-20 bg-gray-800 rounded-full flex items-center justify-center mb-4">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-10 h-10 text-gray-400">
            <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 9.75a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375m-13.5 3.01c0 1.6.8 3.11 2.19 4.024C6.07 18.332 7.5 19.5 9 19.5h6c1.5 0 2.93-1.168 3.99-2.715.32-.297.71-.53 1.13-.69M7.5 11.25v-1.5c0-.8.4-1.6.8-2.4C8.4 6.56 9.5 5.25 11.1 5.25h1.8c1.6 0 2.7 1.31 2.9 2.4.2.8.2 1.6 0 2.4" />
          </svg>
  ...
  return (
    <div className="divide-y divide-gray-800">
      {chats.map((chat) => {
        ...
        return (
          <div key={chat.id} className={`relative group ${isSelected ? 'bg-gray-800' : ''}`}>
            <button 
              className={`w-full flex items-center p-4 hover:bg-gray-800 transition-colors text-left
                ${isSelected ? 'bg-gray-800 border-l-4 border-gray-600' : 'border-l-4 border-transparent'}`}
              onClick={() => onChatSelect(chat.id, chat.productId || '')}
            >
              <div className="relative h-12 w-12 rounded-full overflow-hidden mr-4 flex-shrink-0 shadow-sm border border-gray-700">
                {otherParticipantPhoto ? (
                  ...
                  <div className="fallback-initial-div h-11 w-11 rounded-full bg-gray-800 flex items-center justify-center text-gray-200 font-semibold text-lg">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline">
                  <h3 className={`font-medium truncate ${isSelected ? 'text-gray-200' : 'text-gray-200'}`}>
                ...
                <p className={`text-sm truncate mt-0.5 ${isSelected ? 'text-gray-300' : 'text-gray-400'}`}>
                  {chat.lastMessage ? chat.lastMessage.text : "Chat started"}
                </p>
              </div>
